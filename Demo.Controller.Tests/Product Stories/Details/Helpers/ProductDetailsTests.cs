...
namespace Demo.Controller.Tests.DetailsStory
{
    public partial  class ProductDetailsTests
    {
        ...
        private void AdminLandsOnDetailsPageWithExistingProduct()
        {
            this._repository.Setup(obj => obj.GetProduct(this._id)).Returns(this._product);
            this._controllerResult = this._controller.WithCallTo(c => c.Details(this._id));
        }
        ...
        #region ShouldReturnHttpNotFoundOnDetailsIfNoProductFoundTest

        private void AdminLandsOnDetailsPageWithNonExistingProduct()
        {
